...
class GameState:
    ...
    def make_move(self, index: int, client_id):
        ...
        if self.board[index] is None:
            ...
            self.first_player = self.player1_id > self.player2_id
            self.second_player = self.player1_id < self.player2_id
            self.last_player = client_id
            return True
        
    # def switch_player(self, first_player, second_player):
    ...
    try:
        while True:
            ...
            if game_state.make_move(index, client_id):
                await manager.broadcast(json.dumps({"board": game_state.board, "currentPlayer": game_state.current_player, "firstPlayer": game_state.first_player, "secondPlayer": game_state.second_player}))
